...
**Atualizado:** 2026-04-16 **noite** (Fase 34 Parte A blueprint entregue; fix crítico schema prod + backfill super-admin membership; integração krayin pendente).

> **Fix crítico 2026-04-16 — schema prod defasado + super-admin sem membership (commit fd102d9):**
> Telas em prod retornavam 500 (`custom_attributes does not exist`, `ColumnNotFound` em leads/contacts/opps). Schema prod nunca recebeu migrations das Fases 5/17/24 etc; DB continha só 2 users + 1 company criada manualmente. Resolvido com `prisma db push --force-reset --accept-data-loss` + `tsx prisma/seed.ts` (recria super-admin, system user, 2 companies demo, feature flags, 12 produtos) + SQL backfill das 2 memberships `super_admin` para `nexusai360@gmail.com`. `prisma/seed.ts` agora inclui loop de upsert memberships (preventivo novos envs) + script idempotente `prisma/scripts/backfill-superadmin-membership.ts`. Smoke prod pós-fix: `/api/health` 200, `/login` 200, `/dashboard /leads /contacts /opportunities` 307 (redirect login, esperado).
